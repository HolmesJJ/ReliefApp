...
import com.example.relief.utils.DensityUtils;
...
public class LoadingView extends View {
    ...
    private void drawArc(Canvas canvas) {
        ...
        if (mSweepGradientTopNormal == null) {
            mSweepGradientTopNormal = new SweepGradient(
                    getWidth() >> 1, getWidth() >> 1, new int[]{mEndColorNormal, mStartColorNormal}, new float[]{0f, 0.5f});
        }
        if (mSweepGradientBottomNormal == null) {
            mSweepGradientBottomNormal = new SweepGradient(
                    getWidth() >> 1, getWidth() >> 1, new int[]{mEndColorNormal, mStartColorNormal}, new float[]{0.5f, 1f});
        }
